...
func PCIEDomainsFromFS(sysfs SysFS) ([]PCIEDomain, error) {
	...
	err := ScanPCIDevices(sysfs, func(pciDev PCIEDevice) error {
		...
		pcieRoot := *pcieRootAttr.Value.StringValue
		klog.V(2).Infof("PCIE: bridge at %v PCIE Root %v local CPUs %v", pciDev.Address, pcieRoot, localCPUs.String())

		domains[pcieRoot] = PCIEDomain{
