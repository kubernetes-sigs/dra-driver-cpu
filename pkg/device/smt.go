...
// Compact SMT sibling representation aka `SMTLayout`.
...
// FormatSMTLayout produces a compact string representation of the CPU
// siblings as learned from the given CPUTopology.
func FormatSMTLayout(topo *cpuinfo.CPUTopology) string {
	...
	for _, info := range topo.CPUDetails {
		...
		// note we intentionally add all SMT siblings in the stride slice
		// creating a temporary duplication. CPUIDs will be deduplicated
		// later in the flow by cpuset construction, a step we need anyway.
		// However note we do need to prevent NoSibling to leak into slice.
		if stride == 0 {
	...
	return smtLayout[1:] // strip leading stray AtomSep
}

// DecodeSMTLayout takes a serialized SMT map as produced by
// FormatSMTLayout and returns a map {cpuid -> sibling_cpuid}.
...
func DecodeSMTLayout(data string) (map[int]int, error) {
