...
func NewCPUAllocationStore(provider CPUInfoProvider) *CPUAllocationStore {
	...
	return &CPUAllocationStore{
		allCPUs:          allCPUsSet,
		claimAllocations: make(map[types.UID]cpuset.CPUSet),
	}
}
...
func (s *CPUAllocationStore) AddResourceClaimAllocation(claimUID types.UID, cpus cpuset.CPUSet) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.claimAllocations[claimUID] = cpus
...
func (s *CPUAllocationStore) RemoveResourceClaimAllocation(claimUID types.UID) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.claimAllocations[claimUID]; ok {
		delete(s.claimAllocations, claimUID)
...
func (s *CPUAllocationStore) GetSharedCPUs() cpuset.CPUSet {
	...
	for _, cpus := range s.claimAllocations {
		allocatedCPUs = allocatedCPUs.Union(cpus)
	}
	return s.allCPUs.Difference(allocatedCPUs)
}

// RestoreResourceClaimAllocation restores an allocation to the store during synchronization.
func (s *CPUAllocationStore) RestoreResourceClaimAllocation(claimUID types.UID, cpus cpuset.CPUSet) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.claimAllocations[claimUID] = cpus
	klog.Infof("Restored allocation for resource claim %s: CPUs %s", claimUID, cpus.String())
}
