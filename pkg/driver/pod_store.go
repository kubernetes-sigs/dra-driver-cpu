...
func NewContainerState(cpuType CPUType, containerName string, containerUID types.UID, claimUIDs []types.UID) *ContainerState {
...
// RestoreContainerState restores a container's state in the store during synchronization.
func (s *PodConfigStore) RestoreContainerState(podUID types.UID, state *ContainerState) {
	s.mu.Lock()
	defer s.mu.Unlock()
