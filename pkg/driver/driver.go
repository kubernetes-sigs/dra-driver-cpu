...
type CPUAllocator interface {
	...
	// Validate verifies performed allocation is consistent with the other allocations
	// performed in the same batch (claim asking for two or more allocations) and with all the other
	// active claims. Returns error detailing the inconsistency, nil if the allocation is correct.
	Validate(preferredCPUs, allocatedCPUs, cpuAssignment cpuset.CPUSet) error
}
