...
func init() {
	...
	flag.StringVar(&cpuDeviceMode, "cpu-device-mode", driver.CPU_DEVICE_MODE_GROUPED, "Sets the mode for exposing CPU devices. 'grouped' exposes a single device for a group of CPUs. 'individual' exposes each CPU as a separate device.")
	flag.StringVar(&groupBy, "group-by", driver.GROUP_BY_SOCKET, "When --cpu-device-mode=grouped, sets the criteria for grouping CPUs. Can be 'socket' or 'numanode'.")
}
