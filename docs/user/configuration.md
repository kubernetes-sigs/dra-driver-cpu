...
One release's DaemonSet lands on every node it selects, so those nodes must agree on the
kubelet root; splitting them across releases fails with `invalid ownership metadata`, since
the chart's `DeviceClass` is cluster-scoped under a fixed name. A cluster whose nodes
