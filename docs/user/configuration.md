...
- `numanode`: groups CPUs by NUMA node. Supported with an external allocator (see: `machine`).
- `socket`: groups CPUs by socket. Also supported with an external allocator (see: `machine`).
- `machine`: groups all allocatable node CPUs into a single machine-wide capacity device.
  It requires the external allocator and an external scheduler to supply core assignments; use
  it only when that allocator must select from the entire node. See
