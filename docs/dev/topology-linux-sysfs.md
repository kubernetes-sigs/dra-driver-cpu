...
Still, we can use `numaNode` as "next upper" allocation boundary above the `pcieRoot`
boundary; but as this document hopefully demonstrated till this point, the attribute
is not a straightforward choice nor provides a clear path but is rather the "less worse"
option we can pursue to perform cross-pcieRoot allocation or to link resources inside
the CPU silicon (see 5.3).
...
- `matchAttribute` on `numaNode` matches every device on the socket — it provides
  no placement discrimination whatsoever.
- `matchAttribute` on `pcieRoot` still distinguishes devices on different root complexes.
- A user writing a `numaNode`-based constraint gets a silent no-op: the constraint
  is satisfied but with no actual topology benefit.
