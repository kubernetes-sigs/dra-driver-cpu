...
func TestAffinityScanBoundFromTopology(t *testing.T) {
	t.Run("nil returns default", func(t *testing.T) {
		if got := affinityScanBoundFromTopology(nil); got != affinityScanMax {
			t.Errorf("affinityScanBoundFromTopology(nil) = %d, want %d", got, affinityScanMax)
		}
	})
	t.Run("empty CPUDetails uses NumCPUs", func(t *testing.T) {
		topo := &cpuinfo.CPUTopology{NumCPUs: 16, CPUDetails: cpuinfo.CPUDetails{}}
		if got := affinityScanBoundFromTopology(topo); got != 16 {
			t.Errorf("affinityScanBoundFromTopology() = %d, want 16", got)
		}
	})
	t.Run("max CPU ID plus one", func(t *testing.T) {
		...
		if got := affinityScanBoundFromTopology(topo); got != 10 {
			t.Errorf("affinityScanBoundFromTopology() = %d, want 10", got)
		}
	})
}
